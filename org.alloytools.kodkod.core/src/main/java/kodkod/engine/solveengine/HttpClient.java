...
public class HttpClient {
    ...
    private Model model;
    /** Amount of seconds to wait between each fetchStatus request */
    ...
    /** Booleans to figure out if this instance has already been launched but stopped before the end*/
    ...
    public void setSeStatus(SEStatusCode seStatus) { this.seStatus = seStatus; }
    ...
    /**
     * Do the whole process for solving a problem using the online solver at solve.satalia.com
     * @return the result of the problem formatted in an Helper.SEResults instance
     * @throws SolvingStoppedException if the online solver stopped before ending the job
     * @throws UnusualResponseException if the status received is not usual
     * @throws ServerException if the response got from an https request is unexpected
     * @throws UnsupportedEncodingException if Java fails to encode the problem, should not happen
     * @throws UnvalidTokenException if the token provided is not recognised as valid by SolveEngine
     * @throws UnknownHostException if there is no internet connection
     */
    public Helper.SEResults solve() throws SolvingStoppedException,
            UnusualResponseException,
            ServerException, UnsupportedEncodingException,
            UnvalidTokenException, UnknownHostException, NotImplementedException {
        if (!this.jobCreated) {
            this.createJob();
    ...
    /**
     * Encode the problem data in base 64, then sends it to solve.satalia.com, with the api-key,
     * to create a job (that will be solving the problem when scheduled)
     *
     * @throws UnsupportedEncodingException if Java fails to encode the problem, should not happen
     * @throws ServerException if the response got from the request is unexpected
     * @throws UnknownHostException if there is no internet connection
     * @throws UnvalidTokenException if the token provided is not recognised as valid by SolveEngine
     * @throws NotImplementedException if Java fails to encode the cnf text of the problem, should not happen
     */
    private void createJob() throws UnsupportedEncodingException, ServerException,
            UnvalidTokenException, UnknownHostException, NotImplementedException {
        ...
        Helper.JobCreateResponse resp = (Helper.JobCreateResponse)
                this.sendRequest("POST", "", false, data, Helper.JobCreateResponse.class);

        if (resp.message.length() > 0) {
            if (resp.code.equals("16"))
                throw new UnvalidTokenException("You are using this api-key: " + this.model.getToken()
                        + "\nIt is not recognised. Please check it by signing in at solve.satalia.com, "
                        + "and update it by selecting another solver and then select SolveEngine again");
    ...
    /**
     * Sends a request to schedule a job created precedently. This will actually launch the solver.
     * @throws ServerException if the response got from the request is unexpected
     * @throws UnknownHostException if there is no internet connection
     */
    private void scheduleJob() throws ServerException, UnknownHostException {
        ...
        Helper.SimpleResponse resp =
                this.sendRequest("POST", SEConfig.URL_SCHEDULE, true, "", Helper.SimpleResponse.class);
    ...
    /**
     * Sends a request every sleeping time (2sec by default) to get the status of the solver.
     * @return the status once it is not
     * @throws SolvingStoppedException if the online solver stopped before ending the job
     * @throws UnusualResponseException if the status received is not usual
     * @throws ServerException if an error occurred in the request and the response is not understood
     * @throws UnknownHostException if there is no internet connection ,basically
     */
    private SEStatusCode waitResults() throws SolvingStoppedException,
            UnusualResponseException, ServerException, UnknownHostException {
        Integer secCnt = 0;
        ...
        while (true) {
            ...
            String msg = new StringBuffer().append("Solving the problem, status : ")
                    .append(seStatus).append(" - waiting time : ")
                    .append(secCnt.toString()).append("s")
                    .toString();
            ...
            if (seStatus == SEStatusCode.COMPLETED) {
                ...
                throw new SolvingStoppedException(new StringBuffer()
                        .append("Error with Solve engine :")
                        .append(" the time limit (10min by default)")
                        .append(" has been reached before solving the problem")
                        .toString());
    ...
    private String fetchStatus() throws ServerException, UnknownHostException {
        Helper.JobStatusResponse resp = (Helper.JobStatusResponse)
                this.sendRequest("GET", SEConfig.URL_STATUS, true, "", Helper.JobStatusResponse.class);

        if (resp.message.length() > 0) {
            if (resp.code.equals("5"))
    ...
    private Helper.SEResults fetchSolutions() throws ServerException, UnknownHostException {
        Helper.JobResultsResponse resp = (Helper.JobResultsResponse)
                this.sendRequest("GET", SEConfig.URL_RESULTS, true, "", Helper.JobResultsResponse.class);
    ...
    private void stopJob() throws ServerException, UnknownHostException {
        Helper.SimpleResponse resp =
                this.sendRequest("DELETE", SEConfig.URL_STOP, true, "", Helper.SimpleResponse.class);
    ...
    /**
     * Sends an https request with a baseUrl solve.satalia.com/api/v2/jobs, with json-formatted data
     ...
     * @throws UnknownHostException if there is no internet connection
     */
    public Helper.SimpleResponse sendRequest(String msgType,
                                             String path,
                                             Boolean withJobId,
                                             String data,
                                             Type respModel) throws ServerException, UnknownHostException {
        String toAdd = "";
        if (withJobId) {
            toAdd = "/" + this.jobId;
        }
        String strUrl = new StringBuffer()
                .append(SEConfig.SE_URL_HTTP)
                .append(toAdd).append("/")
                .append(path)
                .toString();
